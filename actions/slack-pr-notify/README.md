...
| `ignore-checks` | não | `weekend-holiday-merge-gate` | Nomes de check-runs a **ignorar** na espera/decisão (vírgula/nova-linha). Não contam como pendente nem falha — útil pra "merge gates" que falham de propósito (não viram ❌ na notificação). Não afeta o branch protection. Default já cobre o gate central. |
